const messages = {
    en: {
        translations: {
            titles: {
                app: 'Skin Gamer Token',
                welcome: 'Welcome to Skin Gamer Token',
                days: 'days',
                hours: 'hours',
                minutes: 'minutes',
                and: 'and',
                preSale: {
                    title: 'Pre Sale in',
                    completed: 'Pre Sale now available'
                },
                launch: {
                    title: '',
                    subtitle: 'to official release',
                    completed: 'Available to buy'
                },
                menuDesktop: {
                    home: 'Home',
                    buy: 'Buy',
                    tokencomics: 'Tokencomics',
                    roadmap: 'Roadmap',
                    nft: 'NFT',
                    platform: 'Platform Skin Token'
                },
                menuMobile: {
                    home: 'Home',
                    buy: 'Buy',
                    tokencomics: 'Tokencomics',
                    roadmap: 'Roadmap',
                    nft: 'NFT (Coming Soon)',
                    platform: 'Platform Skin Token (Coming Soon)',
                    language: 'Language'
                }
            },
            messages: {
                introduction: 'Token was created with the aim of facilitating financial operations in the world of gamers in the skin market, without the need for bureaucracy. We are creating a platform where users will have access to the market of various games to bet or buy game skins like CS:GO, PUBG, TF4 and etc... All this will be possible with this intelligent and decentralized contract.',
                modal: {
                    closeBtn: 'Close',
                    comingSoon: {
                        title: 'Coming Soon',
                        text: 'This option will be available soon, follow our roadmap and follow us on social media for more information.'
                    },
                    langOptions: {
                        title: 'Language',
                        english: 'English',
                        portuguese: 'Português',
                        spanish: 'Español'
                    },
                    launchPcs: {
                        title: 'Coming Soon',
                        text: 'Token purchase by Pancakeswap will only be available on release day (2021/12/10), enter our telegram to stay updated.'
                    }
                }
            }, 
            buttons: {
                buyPcs: 'Buy on PancakeSwap',
                contractBcs: 'Contract BCScan',
                whitepaper: 'Whitepaper',
                chartPoo: 'Chart on PooCoin'   
            },
            tokencomics: {
                title: 'Tokencomics',
                subtitle: 'Supply 100.000.000.000.000',
                preSale: '30% Pre-Sale',
                marketing: '10% Marketing',
                devTeam: '5% Dev Team',
                reward: '5% Reward',
                publicSale: '50% Public Sales'
            },
            roadmap: {
                title: 'Roadmap',
                phase1: {
                    title: 'First Phase',
                    step1: 'Launch on Pancakeswap',
                    step2: 'Liquidity Locked DxSale',
                    step3: 'Website Launch',
                    step4: '1.000 members on Telegram',
                    step5: 'Platform Skin Gamer Token',
                    step6: 'Outreach and mass marketing',
                    step7: 'Closed partnerships with skin sites',
                    step8: 'Listing on the main dissemination platforms'
                },
                phase2: {
                    title: 'Second Phase',
                    step1: 'Search marketing with influencers',
                    step2: 'NFT Competition',
                    step3: 'PooCoin Ads',
                    step4: '2.500 Holders',
                    step5: 'CoinGecko, Coin Hunt and Coin Sniper listings',
                    step6: 'BSC Scam Icon and Trust Wallet',
                    step7: 'Audit with CertiK'
                },
                phase3: {
                    title: 'Third Phase',
                    step1: 'Launch of S.G.Ts Main Platform',
                    step2: 'Maximum boost in our marketing',
                    step3: 'CMC Listing',
                    step4: '5.000 Holders',
                    step5: 'Physical advertising in strategic locations',
                    step6: 'CriptoGames Development',
                    step7: 'CriptoGames Launch'
                }
            }
        }
    }
}

export { messages }